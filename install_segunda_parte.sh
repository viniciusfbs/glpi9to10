...
EOF"
    ...
    log_info "🔄 Executando upgrade para GLPI 10..."
    if docker exec $C_GLPI_10 php /var/www/html/glpi/bin/console glpi:database:update --no-interaction --force; then
        log_success "Upgrade para GLPI 10 concluído com sucesso!"

        # 8. Limpeza de cache
        log_info "🧹 Limpando cache"
        docker exec $C_GLPI_10 rm -rf /var/www/html/glpi/files/_cache/* 2>/dev/null || true
        ...
        log_error "Falha no upgrade para GLPI 10"
         return 1
    fi
}
